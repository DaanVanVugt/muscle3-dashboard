...
@click.option(
    ...
    default=None,
    help="Open a browser at the TCP URL once serving. Default: auto -- on when "
    "a desktop session is detected ($DISPLAY set, e.g. NoMachine) and TCP is "
    "served. Needs TCP.",
)
def serve(
    socket_path: Path,
    ...
    if open_browser is None:
        # Auto: open on a desktop session (e.g. NoMachine sets $DISPLAY) when
        # we're serving TCP for a browser to reach.
        open_browser = bool(tcp_port and os.environ.get("DISPLAY"))
    elif open_browser and not tcp_port:
